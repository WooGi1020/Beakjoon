    ...
    .then((data) => {
      if (data.status === "ok") {
        Array.from(data.articles).forEach((arr) => {
          ...
          img.onerror = () => {
            img.src =
              "https://graphicriver.img.customer.envatousercontent.com/files/413754196/590.jpg?auto=compress%2Cformat&fit=crop&crop=top&w=590&h=590&s=7e250cafe9c20103fdbc966d1bf9de0a";
          };
          img.src =
            arr.urlToImage != null
              ? arr.urlToImage
          ...
          pEl.textContent =
            arr.description != null
              ? arr.description.length > 100
                ? `${arr.description.slice(0, 120)}...`
                : arr.description + "..."
              : "no Content";

          const urlBtn = document.createElement("button");
          urlBtn.textContent = "바로 가기";
